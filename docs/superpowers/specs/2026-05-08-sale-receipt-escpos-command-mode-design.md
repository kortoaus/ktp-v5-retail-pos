...
영수증과 shift settlement Z-report에 ESC/POS 커맨드 기반 출력 모드를 추가한다.
...
- shift close 후 자동 출력되는 Z-report
...
`printShiftSettlementReceipt()`도 같은 config를 읽고 mode에 따라 기존 raster
Z-report 또는 ESC/POS command Z-report로 분기한다.
...
shift -> buildShiftSettlementEscposReceipt(shift, options) -> printESCPOS(bytes)
```
...
Z-report command builder는 기존 raster Z-report와 같은 집계 섹션을 출력한다.

- shift id, business day, opened/closed user and time
- Sales tender totals, GST, total sales
- Refund tender totals and GST
- Net Total by tender and GST
- Cash In / Out
- SPEND retail value when present
- Cash Drawer started/expected/actual/difference
- printed timestamp
...
- 출력 수동 확인:
  ...
  - ESC/POS Z-report가 shift settlement section과 final cut을 출력한다.
...
- 대상 한글 내장 프린터 모델과 code page 번호가 확인되면 code page 선택을
  추가한다.
- production에서 ESC/POS command mode가 승인되면 raster mode 제거 여부를
  검토한다.
