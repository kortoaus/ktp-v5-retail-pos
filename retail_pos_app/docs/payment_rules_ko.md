...
거스름돈이 발생하면 전체 화면 오버레이가 표시되어 거스름돈 금액을 보여줍니다. 캐셔가 현금을 돌려준 후 **Close** 버튼을 눌러 판매를 완료합니다. **Kick Drawer** 버튼으로 캐시 드로어를 다시 열 수도 있습니다.

### 캐시 드로어

`cashPaid > 0`일 때 캐시 드로어가 자동으로 열립니다. ESC/POS 펄스 명령 핀 2 (`ESC p 0x00`)를 사용합니다. 거스름돈 화면에서 수동으로 드로어를 열 수도 있습니다.

### 영수증 출력

결제 완료 후 ESC/POS 감열식 영수증 프린터를 통해 자동으로 영수증이 출력됩니다. 영수증은 **캔버스 이미지** (576px 너비)로 렌더링되어 래스터 비트맵으로 전송됩니다 — 텍스트 모드 ESC/POS가 아닙니다.

영수증 구성 (위에서 아래로):

1. **매장 헤더** — 상호명, 주소, ABN, 전화번호
2. **TAX INVOICE** 라벨
3. **거래 정보** — 영수증 번호, 날짜/시간, 터미널명, 회원 등급
4. **품목 목록** — 각 품목의 수량/중량, 단가, 라인 합계
   - `^` 접두사 = 가격 변경됨 (수정/할인)
   - `#` 접두사 = GST 적용 대상
   - 계량 품목: `0.650KG @ $5.00/KG`
