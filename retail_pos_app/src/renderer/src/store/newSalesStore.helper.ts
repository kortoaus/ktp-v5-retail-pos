  ...
  for (const promotion of promotions) {
    switch (promotion.type) {
      case PromotionType.BUY_MORE_SAVE_MORE: {
        const discount = applyBuyMoreSaveMore(pureLines, promotion);
...
function applyBuyMoreSaveMore(
  lines: SaleLineType[],
  promotion: Promotion,
): SaleStoreDiscount | null {
  const { allowedItemIds, requiredItemIds, discountType } = promotion;
  const targetLines = lines.filter((l) => allowedItemIds.includes(l.itemId));
  const itemPool = [...new Set(targetLines.map((l) => l.itemId))];
  ...
  const totalQty = targetLines.reduce((acc, l) => acc + l.qty, 0);
  const totalPrice = targetLines.reduce((acc, l) => acc + l.total, 0);

  if (totalQty < promotion.minQty) return null;

  const cappedQty =
    promotion.maxQty != null && totalQty > promotion.maxQty
      ? promotion.maxQty
      : totalQty;

  let discountAmount: number;
  if (discountType === "percentage") {
    const pct = promotion.discountPercentAmounts[0] ?? 0;
    discountAmount = Math.round((totalPrice * pct) / PCT_SCALE);
  } else {
    discountAmount = promotion.discountFlatAmounts[0] ?? 0;
  }

  const appliedAmount = Math.round((discountAmount * cappedQty) / totalQty);
  ...
  return {
    ...
    description: targetLines.map((l) => l.name_en).join(", "),
    amount: appliedAmount,
    targetItemIds: allowedItemIds,
  };
}
