...
export default function InterfaceSettingsScreen() {
  ...
  const [zplSerial, setZplSerial] =
    useState<ZplSerialForm>(ZPL_SERIAL_DEFAULTS);
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Toggle
              checked={zplSerial.enabled}
              onChange={(v) => setZplSerial((s) => ({ ...s, enabled: v }))}
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                className={selectClass}
                disabled={!zplSerial.enabled}
                value={zplSerial.language}
                onChange={(e) =>
                  setZplSerial((s) => ({
                    ...s,
                    language: e.target.value as LabelLanguage,
                  }))
                }
              >
            ...
            <div>
              ...
              <select
                className={selectClass}
                disabled={!zplSerial.enabled}
                value={zplSerial.path}
                onChange={(e) =>
                  setZplSerial((s) => ({ ...s, path: e.target.value }))
                }
              >
                <option value="">Select port</option>
                {ports.map((p) => (
