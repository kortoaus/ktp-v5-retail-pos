...
export interface PromotionTagData {
  name_en: string;
  name_ko: string;
  ...
  startDate: string;
  endDate: string;
  allowedItems: { name_en: string; uom: string }[];
}

export function buildPromotionTag7090(
  labelLanguage: LabelLanguage,
  data: PromotionTagData,
): LabelOutput {
  const col2 = 280;
  const lineHeight = 30;
  const charLengthEn = 40;
  ...
  function fmtDiscount(idx: number): string {
    if (data.discountType === "percentage") {
      const val =
        data.discountPercentAmounts[idx] ?? data.discountPercentAmounts[0] ?? 0;
      return `${val / 10}% OFF`;
    }
    const val =
      data.discountFlatAmounts[idx] ?? data.discountFlatAmounts[0] ?? 0;
    return `${fmtMoney(val)} OFF`;
  }

  const gStr = fmtDiscount(0);
  const mStr = fmtDiscount(1);
  const showMember = gStr !== mStr;
  ...
  builder.text(
    10,
    y,
    `[Buy ${data.minQty / QTY_SCALE}+, Get ${gStr}]`,
    41,
    true,
  );
  y += 70;
  ...
  for (const line of splitLines(data.name_ko, 25)) {
    builder.text(10, y, line, 21, true);
    y += lineHeight;
  }

  for (const line of splitLines(data.name_en, charLengthEn)) {
    builder.text(10, y, line, 21, false);
    y += lineHeight;
  }
  ...
  if (showMember) {
    builder.text(10, y, `G ${gStr}`, 41, true);
    builder.text(col2, y, `M ${mStr}`, 41, true);
  } else {
    builder.text(10, y, gStr, 41, true);
  }

  y += 55;

  if (data.maxQty != null) {
    const uom = data.allowedItems[0]?.uom ?? "EA";
    builder.text(10, y, `This promotion applies to a maximum of`, 21, false);
    y += lineHeight;
    builder.text(
      10,
      y,
      `${data.maxQty / QTY_SCALE}${uom} per transaction`,
      21,
      false,
    );
    y += lineHeight;
  ...
  for (const item of data.allowedItems) {
    if (y > 560) break;
    builder.text(10, y, `- ${item.name_en}`, 21, false);
    y += lineHeight;
  }

  const dateRange = fmtDateRangeStr(data.startDate, data.endDate);
  builder.text(10, 630, dateRange, 41, false);
