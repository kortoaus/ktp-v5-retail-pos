...
- Sale math pipeline: `PaymentModal/usePaymentCal.ts` is the central hook (lines + payments → invariants, tax breakdown, cash allocation, paid/remaining/change). `libs/sale/build-payload.ts` turns cart + payment state into `SaleCreatePayload` for `POST /api/sale` / `POST /api/sale/spend`. AU 5¢ rounding is cash-settled: cash-only and voucher+cash can round the cash remainder, but CREDIT/GIFTCARD exact tenders suppress rounding (`round5` helper).
- PaymentModal tender order is voucher-first, cash-second, exact-last. Member carts show Customer Voucher placeholder; non-member carts show User Voucher. Cash can follow voucher, but voucher/cash are locked after CREDIT/GIFTCARD. PaymentModal controls intentionally use div-based tap targets instead of `<button>` so HID scanner Enter suffixes cannot trigger focused buttons.
...
- `app.ts` → Express wiring. Order matters: JSON/cors/logging/health/debug routes first, **then** `terminalMiddleware`, **then** `/api` router, **then** error handler. `/clear` is currently a harmless dev health stub; do not add destructive reset behavior without an explicit gated dev-only flow.
...
- `router.ts` mounts domain modules under `/api/{terminal,shift,item,brand,hotkey,crm,user,sale,printer,cashio,store,cloud,voucher}`. Each module is a folder with `*.router.ts` + `*.controller.ts` + `*.service.ts`. Sale has split create/query/refund/repay/spend services; Cloud has migrate/sync/post/item-sheet sub-services.
- Cloud sync (`v1/cloud/cloud.sync.service.ts`) posts completed invoices and closed shifts to an upstream cloud API via `libs/cloud.api.ts`. `cloudId != null` is the single synced flag; `synced`/`syncedAt` were removed.
- Auth is a bespoke `userId%%%` token scheme (no JWT in practice despite `jsonwebtoken` being in deps). Permission scopes (`admin, sale, interface, user, hotkey, refund, cashio, store, shift`) are enforced per-route; `userId=1` (admin) bypasses scope checks.
