...
### 오프라인 멤버 적립 (hold & sync, specs/2026-08-07)
- [ ] 인터넷 차단(업링크 분리) → 멤버 QR 스캔 → 탑바에 `Offline ****뒷4자리` 표시, level QR 값(없으면 1) 등급가 적용
- [ ] 오프라인 멤버로 판매 완결(현금/카드) → 영수증에 멤버 줄 미출력, PaymentModal 헤더 `Offline · +NP pending` 표시
- [ ] 인터넷 복구 → 다음 판매(또는 서버 재시작)로 싱크 → CRM 잔액 반영 + dmarket 앱 푸시 도착
- [ ] (음성) 온라인 상태에서 존재하지 않는 멤버 QR → alert 거부, 오프라인 부착 아님
- [ ] (음성) 오프라인에서 위조/오타 id 스캔 → 판매는 완결, 복구 후 적립 없음, api-server `CrmInvoicePushOutbox`에 FAILED 잔류
...
- [ ] Cash IO / User 관리: 폼 열린 상태에서 검색 오버레이 띄우면 물리 입력이 **오버레이 필드에만** 들어감 (뒤 폼 금액/코드 필드 오염 없음)
